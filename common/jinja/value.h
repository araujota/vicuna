...
template <typename T> struct extract_pointee {
    using type = T;
};

template <typename U> struct extract_pointee<std::shared_ptr<U>> {
    using type = U;
};
...
template <typename T> bool is_val(const value_t * ptr) {
    using PointeeType = typename extract_pointee<T>::type;
...
template <typename T> typename extract_pointee<T>::type * cast_val(value & ptr) {
    using PointeeType = typename extract_pointee<T>::type;
...
template <typename T_JSON> void global_from_json(context & ctx, const T_JSON & json_obj, bool mark_input);
...
struct value_t {
    ...
    virtual const std::vector<std::pair<value, value>> & as_ordered_object() const {
        throw std::runtime_error(type() + " is not an object value");
    }
    ...
    virtual void insert(const value & /* key */, const value & /* val */) {
        throw std::runtime_error(type() + " is not an object value");
    }
    ...
    virtual value & at(int64_t /* idx */, value & /* default_val */) {
        throw std::runtime_error(type() + " is not an array value");
    }
...
std::string value_to_json(const value &          val,
                          int                    indent   = -1,
                          const std::string_view item_sep = ", ",
...
struct value_hasher {
    size_t operator()(const value & val) const noexcept { return val->unique_hash().digest(); }
};
...
struct value_int_t : public value_t {
    value_int_t(int64_t v) {
        ...
        if (!std::isfinite(val_flt) || val_flt < int64_lower_bound || val_flt >= int64_upper_bound ||
            static_cast<int64_t>(val_flt) != v) {
    ...
    virtual bool as_bool() const override { return val_int != 0; }
    ...
    virtual hasher unique_hash() const noexcept override {
        return hasher(typeid(*this)).update(&val_int, sizeof(val_int)).update(&val_flt, sizeof(val_flt));
    }
  protected:
...
struct value_float_t : public value_t {
    ...
    virtual string as_string() const override {
        ...
        if (out.back() == '.') {
            out.push_back('0');                                       // leave one zero if no decimals
        }
        return out;
    }
...
struct value_bool_t : public value_t {
    ...
    virtual hasher unique_hash() const noexcept override { return val->unique_hash(); }
  protected:
    virtual bool equivalent(const value_t & other) const override {
        return other.is_numeric() && val_int == other.val_int && val_flt == other.val_flt;
    }
...
struct value_array_t : public value_t {
    ...
    value_array_t(value & v) { val_arr = v->val_arr; }

    value_array_t(std::vector<value> && arr) { val_arr = arr; }

    value_array_t(const std::vector<value> & arr) { val_arr = arr; }
    ...
    value pop_at(int64_t index) {
        ...
        if (index < 0 || index >= static_cast<int64_t>(val_arr.size())) {
            throw std::runtime_error("Index " + std::to_string(index) + " out of bounds for array of size " +
                                     std::to_string(val_arr.size()));
        }
    ...
    virtual bool is_hashable() const override {
        if (std::all_of(val_arr.begin(), val_arr.end(),
                        [&](auto & val) -> bool { return val->is_immutable() && val->is_hashable(); })) {
            return true;
        }
        return false;
    }
  ...
  protected:
    virtual bool equivalent(const value_t & other) const override {
        return typeid(*this) == typeid(other) && is_hashable() && other.is_hashable() &&
               std::equal(val_arr.begin(), val_arr.end(), other.val_arr.begin(), value_equivalence());
    }
};
...
struct value_tuple_t : public value_array_t {
    value_tuple_t(value & v) { val_arr = v->val_arr; }

    value_tuple_t(std::vector<value> && arr) { val_arr = arr; }

    value_tuple_t(const std::vector<value> & arr) { val_arr = arr; }
...
struct value_object_t : public value_t {
    ...
    void insert(const std::string & key, const value & val) { insert(mk_val<value_string>(key), val); }
  ...
  protected:
    virtual bool equivalent(const value_t & other) const override {
        return typeid(*this) == typeid(other) && is_hashable() && other.is_hashable() &&
               std::equal(val_obj.begin(), val_obj.end(), other.val_obj.begin(), value_equivalence());
    }
};
...
struct value_none_t : public value_t {
    ...
    virtual hasher unique_hash() const noexcept override { return hasher(typeid(*this)); }
  protected:
    virtual bool equivalent(const value_t & other) const override { return typeid(*this) == typeid(other); }
};
...
struct value_undefined_t : public value_t {
    ...
    virtual std::string type() const override {
        return hint.empty() ? "Undefined" : "Undefined (hint: '" + hint + "')";
    }
    ...
    virtual hasher unique_hash() const noexcept override { return hasher(typeid(*this)); }
  protected:
    virtual bool equivalent(const value_t & other) const override { return is_undefined() == other.is_undefined(); }
};
...
struct func_args {
  public:
    ...
    void ensure_count(size_t min, size_t max = 999) const {
        size_t n = args.size();
        if (n < min || n > max) {
            throw std::runtime_error("Function '" + func_name + "' expected between " + std::to_string(min) + " and " +
                                     std::to_string(max) + " arguments, got " + std::to_string(n));
        }
    }

    template <typename T> void ensure_val(const value & ptr) const {
        if (!is_val<T>(ptr)) {
            throw std::runtime_error("Function '" + func_name + "' expected value of type " +
                                     std::string(typeid(T).name()) + ", got " + ptr->type());
        }
    }

    void ensure_count(bool require0, bool require1, bool require2, bool require3) const {
        static auto bool_to_int = [](bool b) {
            return b ? 1 : 0;
        };
    ...
    template <typename T0> void ensure_vals(bool required0 = true) const {
        ...
        if (required0 && args.size() > 0) {
            ensure_val<T0>(args[0]);
        }
    }
    ...
    template <typename T0, typename T1> void ensure_vals(bool required0 = true, bool required1 = true) const {
        ...
        if (required1 && args.size() > 1) {
            ensure_val<T1>(args[1]);
        }
    }

    template <typename T0, typename T1, typename T2>
    void ensure_vals(bool required0 = true, bool required1 = true, bool required2 = true) const {
        ...
        if (required2 && args.size() > 2) {
            ensure_val<T2>(args[2]);
        }
    }

    template <typename T0, typename T1, typename T2, typename T3>
    void ensure_vals(bool required0 = true, bool required1 = true, bool required2 = true, bool required3 = true) const {
        ...
        if (required3 && args.size() > 3) {
            ensure_val<T3>(args[3]);
        }
    }
  private:
...
struct value_func_t : public value_t {
    ...
    value_func_t(const std::string & name, const func_handler & func) : name(name) { val_func = func; }

    value_func_t(const std::string & name, const func_handler & func, const value & arg_this) :
        name(name),
        arg0(arg_this) {
        val_func = func;
    }
    ...
    virtual std::string as_repr() const override {
        return type() + "<" + name + ">(" + (arg0 ? arg0->as_repr() : "") + ")";
    }
...
struct value_kwarg_t : public value_t {
    ...
    virtual hasher unique_hash() const noexcept override {
        ...
        hash.update(&type_hash, sizeof(type_hash)).update(key.data(), key.size());
        return hash;
    }
  protected:
    virtual bool equivalent(const value_t & other) const override {
