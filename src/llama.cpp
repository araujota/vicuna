...
struct llama_active_lora_params llama_active_lora_default_params() {
    struct llama_active_lora_params result = {
        /*.enabled                    =*/ false,
        /*.host_memory_ratio          =*/ 0.01f,
        /*.device_memory_ratio        =*/ 0.01f,
        /*.min_rank                   =*/ 2,
        /*.max_rank                   =*/ 16,
        /*.train_context_tokens       =*/ 64,
        /*.train_stride_tokens        =*/ 32,
        /*.max_updates_before_rollover =*/ 32,
        /*.adapter_scale              =*/ 1.0f,
...
struct llama_past_lora_params llama_past_lora_default_params() {
    struct llama_past_lora_params result = {};
    result.enabled = false;
    result.gain_max = 1.0f;
    result.gain_decay = 0.05f;
    result.singular_value_floor = 1.0e-4f;

    static const float host_ratio[LLAMA_MEMORY_LORA_BUCKET_COUNT] = { 0.008f, 0.006f, 0.004f, 0.003f, 0.002f };
    static const float device_ratio[LLAMA_MEMORY_LORA_BUCKET_COUNT] = { 0.008f, 0.006f, 0.004f, 0.003f, 0.002f };
    static const uint32_t min_rank[LLAMA_MEMORY_LORA_BUCKET_COUNT] = { 1, 1, 1, 1, 1 };
    static const uint32_t max_rank[LLAMA_MEMORY_LORA_BUCKET_COUNT] = { 8, 8, 8, 8, 8 };
    static const float base_scale[LLAMA_MEMORY_LORA_BUCKET_COUNT] = { 0.75f, 0.55f, 0.40f, 0.28f, 0.18f };
    static const uint64_t half_life_us[LLAMA_MEMORY_LORA_BUCKET_COUNT] = {
        7ull  * 24ull * 60ull * 60ull * 1000000ull,
        30ull * 24ull * 60ull * 60ull * 1000000ull,
        90ull * 24ull * 60ull * 60ull * 1000000ull,
        365ull * 24ull * 60ull * 60ull * 1000000ull,
        3650ull * 24ull * 60ull * 60ull * 1000000ull,
    };
    static const uint64_t condensation_period_us[LLAMA_MEMORY_LORA_BUCKET_COUNT] = {
        7ull  * 24ull * 60ull * 60ull * 1000000ull,
        30ull * 24ull * 60ull * 60ull * 1000000ull,
        90ull * 24ull * 60ull * 60ull * 1000000ull,
        365ull * 24ull * 60ull * 60ull * 1000000ull,
    ...
    static const float merge_source_weight[LLAMA_MEMORY_LORA_BUCKET_COUNT] = { 1.0f, 0.85f, 0.75f, 0.65f, 0.55f };
    static const float merge_target_retention[LLAMA_MEMORY_LORA_BUCKET_COUNT] = { 0.0f, 0.60f, 0.70f, 0.80f, 0.88f };

    for (int i = 0; i < LLAMA_MEMORY_LORA_BUCKET_COUNT; ++i) {
        result.host_memory_ratio[i] = host_ratio[i];
        result.device_memory_ratio[i] = device_ratio[i];
        result.min_rank[i] = min_rank[i];
        result.max_rank[i] = max_rank[i];
        result.base_scale[i] = base_scale[i];
        result.decay_half_life_us[i] = half_life_us[i];
        result.condensation_period_us[i] = condensation_period_us[i];
        result.merge_source_weight[i] = merge_source_weight[i];
        result.merge_target_retention[i] = merge_target_retention[i];
    }
