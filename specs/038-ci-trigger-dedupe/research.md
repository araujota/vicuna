...
- The next push-triggered sanitizer run showed a second real UB path in `common/jinja/value.h`: `value_int_t` still attempted to round-trip large `int64_t` values through `double` using an unchecked cast back to `int64_t`. That constructor needs a range-safe representability check so large JSON integers do not trip UBSan during `global_from_json`.
- The completed `lint-type-quality` run on current HEAD is now failing only because the touched files `common/jinja/runtime.cpp` and `tests/test-jinja.cpp` introduced additional `clang-format` drift beyond the checked-in baseline. Those should be formatted rather than baseline-expanded because the drift is entirely in the newly edited surface.
- The completed `clang-tidy` run on current HEAD is mostly baseline-covered debt, but two residual blockers still need explicit handling: the baseline comparison script must normalize stored baseline paths so `template-instances/../fattn-vec.hpp` and `fattn-vec.hpp` collapse to the same entry, and the new Jinja overflow regression test introduced one extra `readability-static-accessed-through-instance` diagnostic that should be fixed in code rather than recorded in the baseline.
